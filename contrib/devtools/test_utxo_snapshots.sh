...
# since this rule needs to be violated in order to get stohncoind to pick up on
...
# Chosen to try to not interfere with any running stohncoind processes.
SERVER_PORT=8633
...
./src/stohncoind -logthreadnames=1 $SERVER_PORTS \
...
# shellcheck disable=SC2086
./src/stohncoind $SERVER_PORTS -logthreadnames=1 -datadir="$SERVER_DATADIR" \
    $EARLY_IBD_FLAGS -stopatheight="$FINAL_HEIGHT" >/dev/null

echo
echo "-- Starting the server node to provide blocks to the client node..."
# shellcheck disable=SC2086
./src/stohncoind $SERVER_PORTS -logthreadnames=1 -debug=net -datadir="$SERVER_DATADIR" \
...
# shellcheck disable=SC2086
./src/stohncoind $CLIENT_PORTS $ALL_INDEXES -logthreadnames=1 -datadir="$CLIENT_DATADIR" \
...
client_sleep_til_boot
# shellcheck disable=SC2086
./src/stohncoind $CLIENT_PORTS $ALL_INDEXES -logthreadnames=1 -datadir="$CLIENT_DATADIR" -connect=0 \
