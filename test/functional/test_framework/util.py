...
def get_fee(tx_size, feerate_btc_kvb):
    """Calculate the fee in BTC given a feerate is SOH/kvB. Reflects CFeeRate::GetFee"""
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "stohn.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> Tuple[dict, pathlib.Path]:
    ...
    if sys.platform == "win32":
        ...
        datadir = temp_dir / "Stohn"
    else:
        env = dict(HOME=str(temp_dir))
        if sys.platform == "darwin":
            datadir = temp_dir / "Library/Application Support/Stohn"
        else:
            datadir = temp_dir / ".stohn"
    return env, datadir
