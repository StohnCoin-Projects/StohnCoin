...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in stohn.conf or in an included conf file
        ...
        with open(self.nodes[0].datadir_path / 'stohn.conf', 'a', encoding='utf-8') as conf:
    ...
    def test_config_file_log(self):
        ...
        # Write a stohn.conf file in the default data directory containing a
        # datadir= line pointing at the node datadir.
        node = self.nodes[0]
        conf_text = node.bitcoinconf.read_text()
        conf_path = default_datadir / "stohn.conf"
    ...
    def test_ignored_conf(self):
        self.log.info('Test error is triggered when the datadir in use contains a stohn.conf file that would be ignored '
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "stohn.conf" file which is ignored, because a '
        ...
        # the same stohn.conf that would be loaded anyway does not trigger an
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/stohn.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        self.log.info('Test error is triggered when stohn.conf in the default data directory sets another datadir '
                      'and it contains a different stohn.conf file that would be ignored')
        ...
        # stohn.conf that would be ignored.
        node = self.nodes[0]
        (default_datadir / "stohn.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "stohn.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/stohn.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "stohn.conf"
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "stohn.conf"
        ...
        none_existent_conf_file = default_data_dir / "none_existent_stohn.conf"
